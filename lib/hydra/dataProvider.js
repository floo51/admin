...
var _default = (entrypoint, httpClient = _fetchHydra.default, apiDocumentationParser = _apiDocParser.parseHydraDocumentation, useEmbedded = false // remove this parameter for 3.0 (as true)
    ...
    switch (type) {
      ...
      case _reactAdmin.GET_MANY_REFERENCE:
        ...
        return Promise.resolve(response.json["hydra:member"].map(document => transformJsonLdDocumentToReactAdminDocument(document, true, true, useEmbedded))).then(data => Promise.all(data.map(data => convertHydraDataToReactAdminData(resource, data)))).then(data => {
          var _response$json, _response$json2, _response$json$hydra;

          return {
            data,
            total: ((_response$json = response.json) === null || _response$json === void 0 ? void 0 : _response$json["hydra:totalItems"]) || (((_response$json2 = response.json) === null || _response$json2 === void 0 ? void 0 : _response$json2["hydra:view"]) ? ((_response$json$hydra = response.json["hydra:view"]) === null || _response$json$hydra === void 0 ? void 0 : _response$json$hydra["hydra:next"]) ? -2 // there is a next page
            : -1 // no next page
            : -3) // no information
  ...
  return {
    ...
    updateMany: (resource, params) => Promise.all(params.ids.map(id => fetchApi(_reactAdmin.UPDATE, resource, {
      id
    }))).then(() => ({
      data: []
    })),
